...
class MarkovServiceTests(unittest.TestCase):
    ...
    def test_first_history_import_is_capped_at_200_then_fetches_only_new(self):
        messages = [_Message(message_id) for message_id in range(1, 251)]
        ...
        with patch.dict(sys.modules, {"discord": FAKE_DISCORD}):
            self.assertEqual(asyncio.run(service.bootstrap_history()), 200)
            self.assertEqual(self.store.stats()["messages"], 200)
            self.assertEqual(self.store.history_cursor(MARKOV_CHANNEL_ID), 250)

            channel.messages.extend([_Message(251), _Message(252)])
            self.assertEqual(asyncio.run(service.bootstrap_history()), 2)
            self.assertEqual(self.store.stats()["messages"], 202)
