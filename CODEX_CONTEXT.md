...
- `/chart type period username` — public PNG showing the count of active saved numeric release ratings by rating date. `type` supports `daily`, `weekly` (Monday start), `monthly`, and `yearly`; `period` is 1–60 and includes the current incomplete Polish calendar period (`Europe/Warsaw`, including DST). Defaults to `monthly` and 12 periods; omitted `username` resolves the caller's configured Kotone profile. Uses the same dark palette, bundled fonts and avatar cache as `/stats`, includes zero-count periods, and reports ratings skipped because their date is unknown. Colored stacked areas show counts in the same 11 score ranges and colors as `/stats`, with low scores at the bottom and 100 at the top; their sum is the total count, without percentage normalization. The user requested removing the total-line legend caption and the database footer from this chart. No history events or individual Track Ratings are counted, and no user ratings are fetched from AOTY. Shared `time_utils.py` provides Polish presentation time for analytics, profile timestamps, Last.fm absolute scrobble timestamps and AOTY relative date labels. Date-only calendar values remain as supplied, including CSV ratings with synthetic ordering timestamps; stored Unix instants and HTTP protocol dates remain unchanged.
  Chart PNGs are 1280 × 1000 pixels with a wider plotting area and metric cards. Score fills use alpha 72/255 over the dark panel, with bright rounded outlines and antialiasing. All stacked boundaries share convex eased interpolation, preserving exact values at calendar points and preventing overshoot or crossing; a single period uses a rounded stacked column.
