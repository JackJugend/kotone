...
- `/chart type period username` — public PNG showing the count of active saved numeric release ratings by rating date. `type` supports `daily`, `weekly` (Monday start), `monthly`, and `yearly`; `period` is 1–90 and includes the current incomplete Polish calendar period (`Europe/Warsaw`, including DST). Defaults to `monthly` and 12 periods; omitted `username` resolves the caller's configured Kotone profile. Uses the same dark palette, bundled fonts and avatar cache as `/stats`, includes zero-count periods, and reports ratings skipped because their date is unknown. Colored stacked areas show counts in the same 11 score ranges and colors as `/stats`, with low scores at the bottom and 100 at the top; their sum is the total count, without percentage normalization. The user requested removing the total-line legend caption and the database footer from this chart. No history events or individual Track Ratings are counted, and no user ratings are fetched from AOTY. Shared `time_utils.py` provides Polish presentation time for analytics, profile timestamps, Last.fm absolute scrobble timestamps and AOTY relative date labels. Date-only calendar values remain as supplied, including CSV ratings with synthetic ordering timestamps; stored Unix instants and HTTP protocol dates remain unchanged.
  Chart PNGs are 1500 × 1020 pixels with a wider and taller plotting area, centered title, one-row aligned legend and metric cards. The bot avatar is placed in the top-left corner and the selected user avatar in the top-right, both with symmetric 45 px outer margins and a 2 px white circular outline. Score fills use alpha 75/255 over the dark panel, with bright rounded outlines and antialiasing. Series up to 60 periods use convex eased interpolation; denser 61–90 period series use direct linear interpolation so adjacent daily values remain readable and do not form flattened spikes. Both preserve exact values at calendar points and prevent overshoot or crossing. An outline is visible only where its score band has thickness, so the top edge uses the highest score range present at that point. A single period fills the plot width as a rounded stacked area. No white total line or point markers cover the colored areas. The fourth metric shows `average_score`, the mean of exact numeric ratings included in the selected range, with each rating weighted equally; it shows an em dash for an empty range. The separate `average` metric remains the number of ratings per calendar period.
  Every generated analytics graphic exposes relevant SQLite-only filters. `/stats`, `/chart`, `/compare`, and `/wrapped` support release year, genre, release format, score range, review state, like state, rated-tracklist state and artist text. `/ratingdistribution` supports its existing release-year, genre and score filters plus review, like, tracklist and artist filters; its interactive menu remains the release-format selector. Applied filters are written into the image subtitle. Filtering never fetches AOTY and never writes data.
